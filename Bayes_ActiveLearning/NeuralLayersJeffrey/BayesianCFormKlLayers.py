# this function implements the closed form (CForm) KL version of linear and CNN bayesian layers
#!/usr/bin/env python
# -*- coding: utf-8 -*-
...
class _ConvNdGroupNJ(Module):
    ...
    def compute_posterior_params(self):
        weight_var, z_var = self.weight_logvar.exp(), self.z_logvar.exp()
        self.post_weight_var = self.z_mu.pow(2).reshape(-1,1,1,1) * weight_var + z_var.reshape(-1,1,1,1) * self.weight_mu.pow(2) + z_var.reshape(-1,1,1,1) * weight_var
        self.post_weight_mu = self.weight_mu * self.z_mu.reshape(-1,1,1,1)
        return self.post_weight_mu, self.post_weight_var
...
class Conv2dGroupNJ(_ConvNdGroupNJ):
    ...
    def forward(self, x):
        ...
        # return reparametrize(mu_activations * z, (var_activations * z.pow(2)).log(), sampling=self.training,
        #                      cuda=self.cuda)
        return reparametrize(mu_activations * z, (var_activations * z.pow(2)).log(), sampling=True,
                             cuda=self.cuda)
